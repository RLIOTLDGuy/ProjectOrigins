...
savedHistoryFilePath = working_directory + "DebugConsoleData.ini";
...
function _execute_script(args, deferred = false) {
	...
	if (!is_undefined(script)) {
		...
		try {
			...
			response = "-- ERROR: see the debug output for error details --";
		...
		array_push(output, "Wait a sec... Theres no such command: " + consoleString);
		array_push(history, consoleString);
		self._update_positions();
	}
}
